...
contains
  ...
  subroutine get_irmsd_fortran(natoms1,types1_ptr,coords1_ptr, &
    ...
    call canref%shrink()                                                 
    print *, 'false enantiomers possible?: ',rcache%stereocheck  
    ...
    if (checkranks(ref%nat,canref%rank,canmol%rank)) then                
      print *, 'using canonical atom identities as rank backend' 
      rcache%rank(:,1) = canref%rank(:)                                  
      rcache%rank(:,2) = canmol%rank(:)                                  
      if (debug) then                                                    
        print *, 'iRMSD ranks:'                                       
        print *, 'atom',' rank(ref)',' rank(mol)'   
        do i = 1,ref%nat                                                 
          print *, i,rcache%rank(i,1),rcache%rank(i,2)                
        end do                                                           
        print *, ""                                                      
      end if                                                             
    else                                                                 
      !> if not, fall back to atom types                                 
      print *, 'using atom types as rank backend'                
