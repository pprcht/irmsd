...
def _get_sorter_exposed_xyz_fortran() -> ct._CFuncPtr:
    ...
    if _get_sorter_exposed_xyz_fortran is None:
        ...
        f.argtypes = [ct.c_int, ct.c_int, ndpointer(dtype=np.float64, flags='C_CONTIGUOUS'), ndpointer(dtype=np.int32, flags='C_CONTIGUOUS'), ndpointer(dtype=np.int32, flags='C_CONTIGUOUS'), ct.c_double, ct.c_int, ct.c_bool, ct.c_int]
        f.restype = None
        _get_sorter_exposed_xyz_fortran = f
    return _get_sorter_exposed_xyz_fortran

def sorter_exposed_xyz_fortran_raw(nat, nall, xyzall, atall, groups, rthresh, iinversion, allcanon, printlvl) -> None:
